...
- `playwright.config.ts` - Deterministic execution defaults (`workers: 1`, `reuseExistingServer: false`).
- `.prettierrc.json`, `.prettierignore` - Formatting baseline for repository standardization.
- `pnpm run format:check` - Added as an explicit local verification command.
