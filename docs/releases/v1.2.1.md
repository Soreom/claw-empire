...
- Scope: Post-v1.2.0 hardening and maintainability release focused on local verification reliability, runtime safety, type debt reduction, and documentation/API discoverability.
...
### Local Verification and Toolchain Hardening

- **pnpm version alignment**: Standardized the `packageManager` declaration, resolving `ERR_PNPM_BAD_PM_VERSION` class failures.
- **Explicit local validation**: Documented the exact install, format, lint, contract, type, build, browser, and test commands for operators.
- **Explicit build gates**:
  - `pnpm exec tsc -p tsconfig.json --noEmit`
  - `pnpm run build`
- **Formatting enforcement reliability**: Repository-wide Prettier normalization to satisfy `format:check` deterministically during local verification.

### Lint Baseline and Gradual Tightening

- **ESLint Flat Config rollout** with TypeScript and React Hooks coverage in local verification.
...
- **OpenAPI contract checks**:
  - Added `openapi:sync` and `openapi:check` commands to keep spec drift visible and enforceable.
  - Local verification validates OpenAPI structure and fails when `docs/openapi.json` is out of sync.
...
- **Documentation refresh**:
  ...
  - Added the current explicit local validation flow.
